          ...
          {products.map((item, i) => (
            ...
            >
              <motion.div
                ...
                className="group relative rounded-3xl overflow-hidden bg-white border border-slate-200 h-[320px] flex flex-col justify-end p-6 cursor-pointer shadow-sm hover:shadow-xl hover:border-blue-200 transition-all duration-300 block"
              >
                ...
                {/* Text label overlay */}
                <div className="relative z-20 space-y-1.5 bg-white/80 backdrop-blur-sm p-4 -m-2 rounded-2xl border border-slate-100 group-hover:border-blue-100 transition-colors">
                  ...
                  <h4 className="text-base font-black text-slate-900 leading-tight flex justify-between items-center">
                    <span>{item.name}</span>
                    <span className="text-blue-600 text-sm">{item.discountedPrice.toFixed(2)} MAD</span>
                  </h4>
                </div>
              </motion.div>
            </Link>
